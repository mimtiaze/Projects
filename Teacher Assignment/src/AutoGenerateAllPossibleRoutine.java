...
public class AutoGenerateAllPossibleRoutine {
    ...
    public AutoGenerateAllPossibleRoutine(){
        ...
        for(int i=0;tempFaculty!=null;i++){
            if(tempFaculty.lab<tempFaculty.labLimit || tempFaculty.theory<tempFaculty.theoryLimit) {
                ...
                faculties[i].firstSectionOfFaculty = null;
                faculties[i].prevTrimesterDataOfFaculty = null;
                faculties[i].secondPrevTrimesterDataOfFaculty = null;
                faculties[i].thirdPrevTrimesterDataOfFaculty = null;




                CourseToLinkedListOthersInfo temp=tempFaculty.firstSectionOfFaculty;
                while(temp!=null){

                    faculties[i].addCourseWithListAtFirstSectionOfFaculty(temp);
                    faculties[i].addCourseWithListForInitialization(temp);
                    faculties[i].addCourseToDebuggingPointerForInitialization(temp);
                ...
                faculties[i].tempTheoryLimit = tempFaculty.getTheoryLimit();
                faculties[i].tempLabLimit = tempFaculty.getLabLimit();
    ...
    public void facultyOnGoingPart2(){
        ...
        for(int i=0;i<facultyCounter;i++){
            if(!faculties[i].isProcessing){
                if(faculties[i].lab>=faculties[i].labLimit && faculties[i].lab>=faculties[i].labLimit){
                ...
                faculties[i].courseListForDebugging=null;
                faculties[i].tempSectionOfFaculty=null;

                CourseToLinkedListOthersInfo temp=faculties[i].firstSectionOfFaculty;
                while(temp!=null){
                ...
                faculties[i].tempLab = faculties[i].getLab();
                faculties[i].tempTheory = faculties[i].getTheory();
                faculties[i].tempTheoryLimit = faculties[i].getTheoryLimit();
                faculties[i].tempLabLimit = faculties[i].getLabLimit();
                break;
            }
        ...
        if(takeTheRoutine) {
            ...
            boolean duplicateRoutine = false;
            /*// ###################################################################################################################
            // Check if this routine already generated [for avoiding duplicate routine]

            // creating new node
            CompleteRoutineStructureHavingOnlyIndexValue newTempForCheck = new CompleteRoutineStructureHavingOnlyIndexValue();

            // resetting all value
            for (int i = 0; i < facultyCounter; i++) {
                for (int j = 0; j < 20; j++) {                 // I took 20 size of array on that structure
                    newTempForCheck.routines[i][j] = -1;
                }
            }
            ...
            for (int i = 0; i < facultyCounter; i++) {
                int columnTracker = 0;
                newTempForCheck.routines[i][columnTracker++] = i;
                ...
                while (tempListForAddingTheirIndexValue != null) {
                    ...
                    newTempForCheck.routines[i][columnTracker++] = courseIndex;

                    tempListForAddingTheirIndexValue = tempListForAddingTheirIndexValue.next;
                }
            }

            newTempForCheck.next = null;

            CompleteRoutineStructureHavingOnlyIndexValue tempForCheck = routineHeadPointer;
            boolean hereIsTheDuplicate=true;

            while (tempForCheck != null) {
                hereIsTheDuplicate=true;

                for (int i = 0; i < facultyCounter; i++) {
                    for (int j = 0; j < 20; j++) {                 // I took 20 size of array on that structure
                        if (tempForCheck.routines[i][j] != newTempForCheck.routines[i][j]) {
                            hereIsTheDuplicate=false;
                            break;
                        }
                    }
                }

                if(hereIsTheDuplicate){
                    duplicateRoutine=true;
                    break;
                }

                tempForCheck = tempForCheck.next;
            }
    ...
    public void courseOnGoingPart2(FacultyStructureForAutoGenerate facultyOnAction){
        if(facultyOnAction.tempTheory>=facultyOnAction.tempTheoryLimit && facultyOnAction.tempLab>=facultyOnAction.tempLabLimit) {
            facultyOnGoingPart2();
        }
        else{
            for(int i=0;i<courseCounter;i++) {
                if(!courses[i].booked) {
                    ...
                    if (courses[i].islabClass && facultyOnAction.tempLab < facultyOnAction.tempLabLimit) {
                    ...
                    } else if (!courses[i].islabClass && facultyOnAction.tempTheory < facultyOnAction.tempTheoryLimit) {
                        // check conflict for lab class
